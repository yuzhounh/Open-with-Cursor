...
- 本指南使用用户级注册表路径，仅影响当前用户，不需要管理员权限。
...
5. 创建一个名为"Icon"的字符串值，将其设置为Cursor.exe的完整路径(将 `<YourUsername>` 替换为您的实际Windows用户名):
   `C:\Users\<YourUsername>\AppData\Local\Programs\Cursor\Cursor.exe`
6. 在"Cursor"下创建"command"子项。
7. 在"command"中，将"(默认)"值设置为(将 `<YourUsername>` 替换为您的实际Windows用户名):
