...
def add_cursor_menu(key_path, cursor_path, use_v_param=False):
    try:
        ...
        winreg.SetValue(command_key, "", winreg.REG_SZ, f'"{cursor_path}" "{param}"')
...
def main():
    ...
    add_cursor_menu(r"*\shell", cursor_path, use_v_param=False)

    # Add right-click menu for folders
    add_cursor_menu(r"Directory\shell", cursor_path, use_v_param=True)

    # Add right-click menu for folder background
    add_cursor_menu(r"Directory\Background\shell", cursor_path, use_v_param=True)
