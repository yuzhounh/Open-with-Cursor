...
5. Create a string value named "Icon" and set it to the full path of Cursor.exe (replace `<YourUsername>` with your actual Windows username):
   `C:\Users\<YourUsername>\AppData\Local\Programs\Cursor\Cursor.exe`
6. Create a "command" subkey under "Cursor".
7. In "command", set the "(Default)" value to (replace `<YourUsername>` with your actual Windows username):
